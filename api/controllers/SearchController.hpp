#ifndef API_CONTROLLERS_SEARCH_CONTROLLER_HPP
#define API_CONTROLLERS_SEARCH_CONTROLLER_HPP

#include <memory>
#include <boost/beast/http.hpp>
#include "../services/SearchService.hpp"

namespace api {
namespace controllers {

class SearchController {
public:
    explicit SearchController(std::shared_ptr<services::SearchService> service);

    boost::beast::http::response<boost::beast::http::string_body>
    handleSearch(const boost::beast::http::request<boost::beast::http::string_body>& req);

private:
    std::string getQueryParam(const boost::beast::http::request<boost::beast::http::string_body>& req,
                              const std::string& key);
    boost::beast::http::response<boost::beast::http::string_body>
    makeJsonResponse(const boost::beast::http::request<boost::beast::http::string_body>& req,
                     const nlohmann::json& data);

    std::shared_ptr<services::SearchService> service_;
};

} // namespace controllers
} // namespace api

#endif
